...
public class Color implements Serializable {
    ...
    public static Color colorFromHex(String hex) {
        Color color = null;
        try {
            int start = hex.indexOf("#") == 0 ? 1 : 0;
            color = new Color(
                    Integer.valueOf(hex.substring(start, 2 + start), 16),
                    Integer.valueOf(hex.substring(2 + start, 4 + start), 16),
                    Integer.valueOf(hex.substring(4 + start, 6 + start), 16));
        } catch (Exception e) {
            GWT.log(e.toString());
        }
        return color;
